...
export const uploadCategoryFileToStorage = async (
  file: File
): Promise<string> => {
  // Reference to the categories folder
  const storageRef = ref(firebaseStorage, `categories/${file.name}`);
 ...
 */
export const deleteCategoryFileFromStorage = async (
  fileName: string
): Promise<void> => {
  // Reference to the file in the categories folder
  const fileRef = ref(firebaseStorage, `categories/${fileName}`);
  try {
